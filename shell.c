...
main(int argc, char **argv)
{
	...
	while ((cmd_argv[j++] = strtok(NULL, separators)) != NULL)
	    ;
	TtyPrintf(termno, "Is about to fork a child!!!\n");
	pid = Fork();
	...
	if (pid == 0) {
		TtyPrintf(termno, "Is about to execute our program!!!\n");
		TracePrintf(0, "About to exec\n");
	    Exec(cmd_argv[0], cmd_argv);
	    TtyPrintf(termno, "Could not Exec `%s'\n", cmd_argv[0]);
	    Exit(1);
	TtyPrintf(termno, "Is about to wait for child\n");	}
